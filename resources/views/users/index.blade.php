...
@section('content')
    <h1 class="text-2xl font-semibold leading-tigh py-2">Listagem de Usuarios
        <a href="{{ route('users.create')}}" class="bg-blue-900 rounded-full text-white px-4 py-2 text-sm">
            Cria Usuario
        </a>
    </h1>
            ...
            <tbody>
                @foreach($users as $user)
                    <tr>
                        ...
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <a href="{{ route('comments.index', $user->id) }}" class="bg-blue-200 rounded-full py-2 px-6">Comentarios ({{$user->userComments->count()}})</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
